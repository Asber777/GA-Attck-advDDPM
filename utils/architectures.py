...
def get_architecture(model_name="InceptionV3", threat_model='Linf', cifar10=False) -> torch.nn.Module:
    ...
    if not cifar10:
        ...
        ## Other models
            ...
            try: 
                robust_path = "/root/hhtpro/123/models"
                model = load_model(model_name,dataset='imagenet', threat_model=threat_model, model_dir=robust_path)
                model.input_size = 256
                normalize_layer = NormalizeLayer(mean=NO_MEAN, std=NO_STD)
            except:
                raise Exception("Not Supported Model Name!")
    else:
        robust_path = "/root/hhtpro/123/models"
        model = load_model(model_name,dataset='cifar10', threat_model=threat_model, model_dir=robust_path)
        model.input_size = 32
        normalize_layer = NormalizeLayer(mean=NO_MEAN, std=NO_STD)
