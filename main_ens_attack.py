...
parser.add_argument('--attack_method', type=str, default="GA_DMI_FSA")
parser.add_argument('--save_path', type=str, default="tmp/images")
parser.add_argument('--mode', type=str, default="nearest")
parser.add_argument('--loss_fn', type=str, default="margin")#ce
parser.add_argument('--momentum', default=1.0, type=float,help='momentum, (default: 1.0)')
parser.add_argument('--epsilon', default=3.5, type=float,help='perturbation, (default: 16)')
parser.add_argument('--max_epsilon', default=3.5, type=float,help='perturbation, (default: 16)')
parser.add_argument('--intervals', default=5, type=int,help='number of intervals')
parser.add_argument('--num_steps', default=50, type=int,help='number of steps in TDMI')
...
parser.add_argument('--target_m', default="Salman2020Do_50_2", type=str,help='robustbench model to be attacked')
...
parser.add_argument('--thres', default=0.3, type=float, help='threshold for continue attack')
...
def main(args):
    ...
    if args.target_m is not None:
        tmp_model = get_architecture(model_name=args.target_m).cuda().eval()
    ...
    for (img, label, img_name) in attack_loader:
        ...
        for i in range(batch_szie):
            ...
            torch.save(x_adv_cpu, args.save_path + "/" + img_name[i]+'.pt')
    ...
    if not args.distributed or args.local_rank == 0:
        ...
        logger.log("Source Success total: ", pgd_err_total)
