...
from utils import get_architecture, Input_diversity, MultiEnsemble
from utils import CrossEntropyLoss, MarginLoss
from attacks.l2 import GA_TDMI_fgsm_L2
...
parser.add_argument('--epsilon', default=255*10, type=float,help='perturbation, (default: 16)')
parser.add_argument('--max_epsilon', default=255*10, type=float,help='perturbation, (default: 16)')
...
parser.add_argument('--target_m', default="Rebuffi2021Fixing_70_16_cutmix_extra", type=str,help='robustbench model to be attacked')
...
def main(args):
    ...
    for idx in auxiliary_id_list:
        temp_model = get_architecture(model_name=MODEL_NAME_DICT[idx], threat_model=args.threat_model, cifar10=True).cuda().eval()
        Auxiliary_model_list.append(temp_model)

    Auxiliary_model = MultiEnsemble(Auxiliary_model_list, args=args, 
        num_classes=NUM_CLASSES, prob=args.prob, mode=args.mode, diversity_scale=args.scale)
    ...
    count = 0
    for (img, label) in attack_loader:
        ...
        if args.attack_method == "TDI_fgsm":
        ...
        elif args.attack_method == 'GA_TDMI_fgsm_L2':
            x_adv, budget = GA_TDMI_fgsm_L2(Source_model, Auxiliary_model, img.clone(), label.clone(), args, loss_fn)
            ...
            x_adv, budget = GA_DMI_FSA(Source_model, Auxiliary_model, img.clone(), label.clone(), args, loss_fn,size=size)
        ...
        if distance_batch.sum() == 0:
            logger.log(f"Attacked: {count+batch_szie}, Batch size: {batch_szie}"+\
                f"Image name: {str(label.cpu().numpy())}, Nature error: {err.item()},"+\
                f"Target error: {err_target.item()}, Target error mask: {err_mask},"+\
                f"budgets: {0}, Batch distance Max: {0},"+\
                f"Avg: {0}, Avg reward: {0}, Lp: {0}")
        else:
            ...
            logger.log(f"Attacked: {count+batch_szie}, Batch size: {batch_szie},"+\
                    f"Image name: {str(label.cpu().numpy())}, Nature error: {err.item()}, "+\
                    f"Target error: {err_target.item()}, Target error mask: {err_mask}, "+\
                    f"budgets: {budget}, Batch distance Max: {distance_batch.max().item()}, "+\
                    f"Avg: {distance_batch.mean().item()}, Avg reward: {batch_score.mean().item()}"+\
                    f", L2: {Lp_dis.data.sum()}")
        budget_cpu = budget.detach()
        budget_cpu[~err_mask] *= -1
        budget_cpu = budget_cpu.cpu().numpy()
        torch.save(torch.cat([x_adv, img]), image_path+ "/" + str(count)+'.pt')

        for i in range(batch_szie):
            x_adv_cpu = x_adv[i, :, :, :].cpu()
            img_adv = transforms.ToPILImage()(x_adv_cpu).convert('RGB')
            img_adv.save(image_path + "/" + str(count + i)+'.png')
            ...
            save_diff = np.reshape(save_diff, newshape=[size*3, size, 3])
            save_diff = save_diff.astype(np.uint8)
            Image.fromarray(save_diff, mode='RGB').save(diff_path + "/" + str(count + i)+'.png')
            img_budget[str(label[i].cpu().item())] = budget_cpu[i]
        count += batch_szie
    ...
    if not args.distributed or args.local_rank == 0:
        ...
        logger.log('Avg L2_distance: {}'.format((Lp_dis_total / count)))
