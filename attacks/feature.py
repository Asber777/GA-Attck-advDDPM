...
def GA_DMI_FSA(model, eval_model, x_nature, y, args, loss_fn,size=299):
    batch_size = x_nature.shape[0]
    ...
    for eps in eps_list:
        ...
        for idx in range(num_steps):
            ...
            with torch.enable_grad():
                model_input = F.interpolate(generated_adv_img_rescaled, size=size, mode="nearest")
                logits = model(model_input, diversity=True)
                adv_loss = loss_fn(logits, y)
    ...
    return F.interpolate(generated_adv_img, size=size, mode="nearest"), budget
